...
export default function* attackReveal() {
    ...
    // yield takeEvery(ActionType.ATTACK_BROADCAST, attackReceiveBroadcastChannel);
}
...
function* attackBroadcast(action: ReturnType<typeof Action.attackBroadcast>) {
    console.log("in attack broadcast")
...
function* revealInput(action: ReturnType<typeof Action.revealInput>) {
    ...
    if (action.payload.reveal === Reveal.Hit || action.payload.reveal === Reveal.Miss) {
        ...
        // it was a sink
        // we need to do this test as we have nullable values - this probably shouldnt be the case
        if (
            action.payload.x1 &&
            action.payload.y1 &&
            action.payload.x2 &&
            ...
            throw new Error("missing components in battleship sink");
        }
    }
...
function* revealBroadcast(action: ReturnType<typeof Action.revealBroadcast>) {
    // they've reveal to us, lets transition to awaiting the
    console.log("made it to reveal broadcast")
    // TODO: fill in a record here
